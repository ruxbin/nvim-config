...
--    print(k)
    if k=="background" or k=="bg" then
    	result["bg"] = string.format("#%06x", v)
    end
    if k=="foreground" or k=="fg" then
	result["fg"] = string.format("#%06x", v)
    end
  end
  return result
end
...
M.gen_highlights = function()
  ...
  M.colors = c
  local groups = {
    RuxbinAdd = {bg = c.white,fg=c.black,style="bold"},
